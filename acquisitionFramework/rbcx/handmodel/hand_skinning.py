...
# 保留所有权利。
#
# 本源代码采用本代码仓根目录 LICENSE 文件中所述的许可协议。

"""手部蒙皮与前向运动学。

该文件实现从关节角到关键点 / 网格顶点的局部变换、线性混合蒙皮和
批量前向运动学，是 emg2pose 三维几何计算的底层支撑。
"""
...
# 注意：相关推导可参考 `EMG2POSE_DEVELOPER_NOTE.md`
# 原始实现可对应到 `pytorch3d.transforms.so3_exp_map`
...
def _finger_fk(
    ...
    每根手指包含 4 个自由度 / 关节，这里返回其中 3 个中间变换帧。
    输入：
...
def _get_skinning_weights(
    ...
    # 为每个 batch / landmark 线性偏移骨骼索引，便于直接写入展平后的权重矩阵。
...
def _skin_points(
    ...
    # 这里通过展平前导维来支持任意 batch 形状，而不把实现绑定到某一种固定维度。
